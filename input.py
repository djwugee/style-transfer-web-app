...
def video_input(style_model_name):
    ...
    if st.sidebar.checkbox('Upload'):
        ...
        video_name = st.sidebar.selectbox("Choose the video:", video_files_name)
    ...
    if video_file is not None:
        ...
        st.warning("Upload a Video OR Untick the Upload Button")
        st.stop()

    WIDTH = st.sidebar.select_slider('QUALITY (May reduce the speed)', list(range(150, 501, 50)), value=200)

    class NeuralStyleTransferVideoTransformer(VideoTransformerBase):
        _width = WIDTH
    ...
    ctx = webrtc_streamer(
        ...
        video_transformer_factory=NeuralStyleTransferVideoTransformer,
...
def webcam_input(style_model_name):
    ...
    class NeuralStyleTransferWebcamTransformer(VideoTransformerBase):
        _width = WIDTH
        _model_name = style_model_name
        _model = None
    ...
    ctx = webrtc_streamer(
        ...
        video_transformer_factory=NeuralStyleTransferWebcamTransformer,
        key="neural-style-transfer",
    )
